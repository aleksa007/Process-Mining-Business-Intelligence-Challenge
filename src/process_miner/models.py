...
def baseline(data_train, data_test):
    ...
    log: Dict[Union[str, List[str]], List[list]]
    log_test: Dict[Union[str, List[str]], List[list]]

    log, log_test = load_base_dict(data_train, data_test)
...
def combs_algo(data_train, data_test):
    ...
    log, log_test = load_comb_dict(data_train, data_test)
    ...
    log_test, data_test = drop_bugs(log_test, data_test)
    ...
    for i in pbar(log_test.keys()):  # check
        ID = []
        stamps = []
        for pairID, stamp in zip(pairwise(log_test[i][0]), pairwise(log_test[i][1])):
            ID.append(pairID)
            stamps.append(stamp)

        log_test[i].append(ID)
        log_test[i].append(stamps)
    ...
    for i in pbar(log_test.keys()):  # check
        count = 0
        for perm in log_test[i][3]:
            index = log_test[i][3].index(perm)
            if index == 0:
                log_test[i][3].insert(0, ('New Event', log_test[i][3][index][0]))

        for count in range(len(log_test[i][4])):
            stamp = log_test[i][4][count]
            if count == 0:
                log_test[i][4].insert(0, (stamp[0], stamp[0]))
    ...
    for i in pbar(log_test.keys()):  # check
        # Add the real time differences
        real_diff = []
        for t in log_test[i][4]:
            real_diff.append(timeDiff(t, datetimeFormat))
        log_test[i].extend([real_diff])
    ...
    for i in pbar(log_test.keys()):  # check
        current = log_test[i][3]
        prediction = []

        for perm in current:
            index = current.index(perm)
            day = int(log_test[i][2][index])
            current_prediction = comb_times[perm][day]
        ...
        log_test[i].extend([prediction])

        current_real = []

        for x in log_test[i][0]:
            if log_test[i][0].index(x) == 0:
                current_real.append('New Event')
            else:
                current_real.append(x)
        log_test[i].extend([current_real])
    ...
    for i in pbar(log_test.keys()):  # check
        time_pred = []
        avg_time_lst = []
        for ev, pred in zip(log_test[i][0], log_test[i][6]):
            last = len(log_test[i][0]) - 1

            if log_test[i][0].index(ev) == last:
    ...
    for i in log_test.keys():
        for x in range(len(log_test[i][0])):
            ...
            event_names.append(log_test[i][0][x])
            timestamp.append(log_test[i][1][x])
            p_event.append(log_test[i][6][x])
            current_real.append(log_test[i][7][x])

            real_diff.append(log_test[i][5][x])
            pred_diff.append(log_test[i][8][x])
...
def de_tree(data_train, data_test):
    ...
    log, log_test = load_tree_dict(data_train, data_test, cases)
    ...
    #  new dictionary that will contain for every position(key) the observed traces,
    #  and next events for each trace(values)
    ...
    def update_tree(case):
        ...
        count = 0
        for _ in case:  # case is an array
            if count not in train_data:  # making the two lists in the dictionary
                # list 1 is all for all traces of the position, list 2 is for all next events
    ...
    # TIMESTAMPS REGRESSION or LightGBM
    ...
    if not lg:
        ...
        df_train = pd.get_dummies(df_train,
                                  columns=['event', 'prev_event', 'week_day_prev', 'position_event', 'lifecycle'])
